...
class GFPersian_Gateway_navaco {
        ...
        if ( is_admin() && self::has_access() ) {
            ...
            if ( rgget( "page" ) == "gf_settings" ) {
                RGForms::add_settings_page( array(
                                                'name'      => 'gf_navaco',
                                                'tab_label' => __( 'درگاه ناواکو', 'gravityformsnavaco' ),
                                                'title'     => __( 'تنظیمات درگاه ناواکو', 'gravityformsnavaco' ),
    ...
    public static function admin_notice_persian_gf() {
        ...
        $message = sprintf( __( "برای استفاده از نسخه جدید درگاه های پرداخت گرویتی فرم نصب بسته فارسی ساز نسخه 2.3.1 به بالا الزامی است. برای نصب فارسی ساز %sکلیک کنید%s.", "gravityformsnavaco" ), '<a href="' . admin_url( "plugin-install.php?tab=plugin-information&plugin=persian-gravity-forms&TB_iframe=true&width=772&height=884" ) . '">', '</a>' );
        printf( '<div class="%1$s"><p>%2$s</p></div>', $class, $message );
    }

    public static function admin_notice_gf_support() {
        $class   = 'notice notice-error';
        $message = sprintf( __( "درگاه ناواکو نیاز به گرویتی فرم نسخه %s به بالا دارد. برای بروز رسانی هسته گرویتی فرم به %sسایت گرویتی فرم فارسی%s مراجعه نمایید .", "gravityformsnavaco" ), self::$min_gravityforms_version, "<a href='http://gravityforms.ir/11378' target='_blank'>", "</a>" );
        printf( '<div class="%1$s"><p>%2$s</p></div>', $class, $message );
    }
    ...
    public static function gravityformsnavaco( $form, $entry ) {
        $navaco = array(
            ...
            'title' => __( 'ناواکو', 'gravityformsnavaco' ),
            'param' => array(
                'email'  => __( 'ایمیل', 'gravityformsnavaco' ),
                'mobile' => __( 'موبایل', 'gravityformsnavaco' ),
                'desc'   => __( 'توضیحات', 'gravityformsnavaco' )
            )
        );
    ...
    public static function tooltips( $tooltips ) {
        $tooltips["gateway_name"] = __( "تذکر مهم : این قسمت برای نمایش به بازدید کننده می باشد و لطفا جهت جلوگیری از مشکل و تداخل آن را فقط یکبار تنظیم نمایید و از تنظیم مکرر آن خود داری نمایید .", "gravityformsnavaco" );
    ...
    public static function menu( $menus ) {
        ...
        if ( ! empty( $permission ) ) {
            $menus[] = array(
                "name"       => "gf_navaco",
                "label"      => __( "ناواکو", "gravityformsnavaco" ),
    ...
    public static function uninstall() {
        if ( ! self::has_access( "gravityforms_navaco_uninstall" ) ) {
            die( __( "شما مجوز کافی برای این کار را ندارید . سطح دسترسی شما پایین تر از حد مجاز است . ", "gravityformsnavaco" ) );
        }
    ...
    public static function feed_page() {
        ...
        <h3>
			<span><i class="fa fa-credit-card"></i> <?php esc_html_e( 'ناواکو', 'gravityformsnavaco' ) ?>
                <a id="add-new-confirmation" class="add-new-h2"
                   href="<?php echo esc_url( admin_url( 'admin.php?page=gf_navaco&view=edit&fid=' . absint( rgget( "id" ) ) ) ) ?>"><?php esc_html_e( 'افزودن فید جدید', 'gravityformsnavaco' ) ?></a></span>
            <a class="add-new-h2"
               href="admin.php?page=gf_navaco&view=stats&id=<?php echo absint( rgget( "id" ) ) ?>"><?php _e( "نمودار ها", "gravityformsnavaco" ) ?></a>
        </h3>
    ...
    private static function list_page( $arg ) {
        ...
        if ( ! self::is_gravityforms_supported() ) {
            die( sprintf( __( "درگاه ناواکو نیاز به گرویتی فرم نسخه %s دارد. برای بروز رسانی هسته گرویتی فرم به %sسایت گرویتی فرم فارسی%s مراجعه نمایید .", "gravityformsnavaco" ), self::$min_gravityforms_version, "<a href='http://gravityforms.ir/11378' target='_blank'>", "</a>" ) );
        }
        ...
        if ( rgpost( 'action' ) == "delete" ) {
            ...
            <div class="updated fade"
                 style="padding:6px"><?php _e( "فید حذف شد", "gravityformsnavaco" ) ?></div><?php
            ...
            ?>
            <div class="updated fade"
                 style="padding:6px"><?php _e( "فید ها حذف شدند", "gravityformsnavaco" ) ?></div>
            <?php
        }
        ?>
        <div class="wrap">
            ...
            <?php if ( $arg != 'per-form' ) { ?>
                ...
                <h2>
                    <?php _e( "فرم های ناواکو", "gravityformsnavaco" );
                    if ( get_option( "gf_navaco_configured" ) ) { ?>
                        <a class="add-new-h2"
                           href="admin.php?page=gf_navaco&view=edit"><?php _e( "افزودن جدید", "gravityformsnavaco" ) ?></a>
                        <?php
                    } ?>
                </h2>
            ...
            <form id="confirmation_list_form" method="post">
                ...
                <div class="tablenav">
                    <div class="alignleft actions" style="padding:8px 0 7px 0;">
                        <label class="hidden"
                               for="bulk_action"><?php _e( "اقدام دسته جمعی", "gravityformsnavaco" ) ?></label>
                        <select name="bulk_action" id="bulk_action">
                            <option value=''> <?php _e( "اقدامات دسته جمعی", "gravityformsnavaco" ) ?> </option>
                            <option value='delete'><?php _e( "حذف", "gravityformsnavaco" ) ?></option>
                        </select>
                        <?php
                        echo '<input type="submit" class="button" value="' . __( "اعمال", "gravityformsnavaco" ) . '" onclick="if( jQuery(\'#bulk_action\').val() == \'delete\' && !confirm(\'' . __( "فید حذف شود ؟ ", "gravityformsnavaco" ) . __( "\'Cancel\' برای منصرف شدن, \'OK\' برای حذف کردن", "gravityformsnavaco" ) . '\')) { return false; } return true;"/>';
                        ?>
                        <a class="button button-primary"
                           href="admin.php?page=gf_settings&subview=gf_navaco"><?php _e( 'تنظیمات حساب ناواکو', 'gravityformsnavaco' ) ?></a>
                    </div>
                </div>
                <table class="wp-list-table widefat fixed striped toplevel_page_gf_edit_forms" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <th scope="col" id="active" class="manage-column"
                            style="width:<?php echo $arg != 'per-form' ? '50px' : '20px' ?>"><?php echo $arg != 'per-form' ? __( 'وضعیت', 'gravityformsnavaco' ) : '' ?></th>
                        <th scope="col" class="manage-column"
                            style="width:<?php echo $arg != 'per-form' ? '65px' : '30%' ?>"><?php _e( " آیدی فید", "gravityformsnavaco" ) ?></th>
                        <?php if ( $arg != 'per-form' ) { ?>
                            <th scope="col"
                                class="manage-column"><?php _e( "فرم متصل به درگاه", "gravityformsnavaco" ) ?></th>
                        <?php } ?>
                        <th scope="col" class="manage-column"><?php _e( "نوع تراکنش", "gravityformsnavaco" ) ?></th>
                    </tr>
                    </thead>
                    <tfoot>
                    <tr>
                        ...
                        <th scope="col" id="active"
                            class="manage-column"><?php echo $arg != 'per-form' ? __( 'وضعیت', 'gravityformsnavaco' ) : '' ?></th>
                        <th scope="col" class="manage-column"><?php _e( "آیدی فید", "gravityformsnavaco" ) ?></th>
                        <?php if ( $arg != 'per-form' ) { ?>
                            <th scope="col"
                                class="manage-column"><?php _e( "فرم متصل به درگاه", "gravityformsnavaco" ) ?></th>
                        <?php } ?>
                    ...
                    if ( ! get_option( "gf_navaco_configured" ) ) {
                        ...
                        <tr>
                            <td colspan="5" style="padding:20px;">
                                <?php echo sprintf( __( "برای شروع باید درگاه را فعال نمایید . به %sتنظیمات ناواکو%s بروید . ", "gravityformsnavaco" ), '<a href="admin.php?page=gf_settings&subview=gf_navaco">', "</a>" ); ?>
                            </td>
                        </tr>
                        <?php
                        ...
                        foreach ( $settings as $setting ) {
                            ...
                            <tr class='author-self status-inherit' valign="top">
                                ...
                                <td><img style="cursor:pointer;width:25px"
                                         ...
                                         alt="<?php echo $setting["is_active"] ? __( "درگاه فعال است", "gravityformsnavaco" ) : __( "درگاه غیر فعال است", "gravityformsnavaco" ); ?>"
                                         title="<?php echo $setting["is_active"] ? __( "درگاه فعال است", "gravityformsnavaco" ) : __( "درگاه غیر فعال است", "gravityformsnavaco" ); ?>"
                                ...
                                <td><?php echo $setting["id"] ?>
                                    <?php if ( $arg == 'per-form' ) { ?>
                                        <div class="row-actions">
                                                <span class="edit">
                                                    <a title="<?php _e( "ویرایش فید", "gravityformsnavaco" ) ?>"
                                                       href="admin.php?page=gf_navaco&view=edit&id=<?php echo $setting["id"] ?>"><?php _e( "ویرایش فید", "gravityformsnavaco" ) ?></a>
                                                    |
                                                </span>
                                            <span class="trash">
                                                    <a title="<?php _e( "حذف", "gravityformsnavaco" ) ?>"
                                                       href="javascript: if(confirm('<?php _e( "فید حذف شود؟ ", "gravityformsnavaco" ) ?> <?php _e( "\'Cancel\' برای انصراف, \'OK\' برای حذف کردن.", "gravityformsnavaco" ) ?>')){ DeleteSetting(<?php echo $setting["id"] ?>);}"><?php _e( "حذف", "gravityformsnavaco" ) ?></a>
                                                </span>
                                        </div>
                                    <?php } ?>
                                </td>
                                ...
                                <?php if ( $arg != 'per-form' ) { ?>
                                    <td class="column-title">
                                        <strong><a class="row-title"
                                                   ...
                                                   title="<?php _e( "تنظیم مجدد درگاه", "gravityformsnavaco" ) ?>"><?php echo $setting["form_title"] ?></a></strong>
                                        ...
                                        <div class="row-actions">
                                            ...
                                            <span class="trash">
                                                <a title="<?php _e( "حذف فید", "gravityformsnavaco" ) ?>"
                                                   href="javascript: if(confirm('<?php _e( "فید حذف شود؟ ", "gravityformsnavaco" ) ?> <?php _e( "\'Cancel\' برای انصراف, \'OK\' برای حذف کردن.", "gravityformsnavaco" ) ?>')){ DeleteSetting(<?php echo $setting["id"] ?>);}"><?php _e( "حذف", "gravityformsnavaco" ) ?></a>
                                                |
                                            </span>
                                            <span class="view">
                                                <a title="<?php _e( "ویرایش فرم", "gravityformsnavaco" ) ?>"
                                                   href="admin.php?page=gf_edit_forms&id=<?php echo $setting["form_id"] ?>"><?php _e( "ویرایش فرم", "gravityformsnavaco" ) ?></a>
                                                |
                                            </span>
                                            <span class="view">
                                                <a title="<?php _e( "مشاهده صندوق ورودی", "gravityformsnavaco" ) ?>"
                                                   href="admin.php?page=gf_entries&view=entries&id=<?php echo $setting["form_id"] ?>"><?php _e( "صندوق ورودی", "gravityformsnavaco" ) ?></a>
                                                |
                                            </span>
                                            <span class="view">
                                                <a title="<?php _e( "نمودارهای فرم", "gravityformsnavaco" ) ?>"
                                                   href="admin.php?page=gf_navaco&view=stats&id=<?php echo $setting["form_id"] ?>"><?php _e( "نمودارهای فرم", "gravityformsnavaco" ) ?></a>
                                            </span>
                                        </div>
                                    </td>
                                <?php } ?>
                                ...
                                <td class="column-date">
                                    ...
                                    if ( isset( $setting["meta"]["type"] ) && $setting["meta"]["type"] == 'subscription' ) {
                                        _e( "عضویت", "gravityformsnavaco" );
                                    } else {
                                        _e( "محصول معمولی یا فرم ارسال پست", "gravityformsnavaco" );
                                    }
                                    ?>
                                </td>
                            </tr>
                        ...
                        <tr>
                            <td colspan="5" style="padding:20px;">
                                <?php
                                if ( $arg == 'per-form' ) {
                                    echo sprintf( __( "شما هیچ فید ناواکوی ندارید . %sیکی بسازید%s .", "gravityformsnavaco" ), '<a href="admin.php?page=gf_navaco&view=edit&fid=' . absint( rgget( "id" ) ) . '">', "</a>" );
                                } else {
                                    echo sprintf( __( "شما هیچ فید ناواکوی ندارید . %sیکی بسازید%s .", "gravityformsnavaco" ), '<a href="admin.php?page=gf_navaco&view=edit">', "</a>" );
                                }
                                ?>
                            </td>
                        </tr>
        ...
        <script type="text/javascript">
            ...
            function ToggleActive(img, feed_id) {
                ...
                if (is_active) {
                    ...
                    jQuery(img).attr('title', '<?php _e( "درگاه غیر فعال است", "gravityformsnavaco" ) ?>').attr('alt', '<?php _e( "درگاه غیر فعال است", "gravityformsnavaco" ) ?>');
                }
                else {
                    img.src = img.src.replace("active0.png", "active1.png");
                    jQuery(img).attr('title', '<?php _e( "درگاه فعال است", "gravityformsnavaco" ) ?>').attr('alt', '<?php _e( "درگاه فعال است", "gravityformsnavaco" ) ?>');
                }
                ...
                mysack.onError = function () {
                    alert('<?php _e( "خطای Ajax رخ داده است", "gravityformsnavaco" ) ?>')
                };
                mysack.runAJAX();
                return true;
            }
    ...
    public static function payment_entry_detail( $form_id, $entry ) {
        ...
        if ( ! empty( $payment_gateway ) && $payment_gateway == "navaco" ) {
            ...
            <hr/>
            <strong>
                <?php _e( 'اطلاعات تراکنش :', 'gravityformsnavaco' ) ?>
            </strong>
            <br/>
            <br/>
            <?php
            ...
            if ( $payment_status == 'Paid' ) {
                $payment_status_persian = __( 'موفق', 'gravityformsnavaco' );
            }
            ...
            if ( $payment_status == 'Cancelled' ) {
                $payment_status_persian = __( 'منصرف شده', 'gravityformsnavaco' );
            }

            if ( $payment_status == 'Failed' ) {
                $payment_status_persian = __( 'ناموفق', 'gravityformsnavaco' );
            }

            if ( $payment_status == 'Processing' ) {
                $payment_status_persian = __( 'معلق', 'gravityformsnavaco' );
            }

            if ( ! strtolower( rgpost( "save" ) ) || RGForms::post( "screen_mode" ) != "edit" ) {
                echo __( 'وضعیت پرداخت : ', 'gravityformsnavaco' ) . $payment_status_persian . '<br/><br/>';
                echo __( 'تاریخ پرداخت : ', 'gravityformsnavaco' ) . '<span style="">' . $payment_date . '</span><br/><br/>';
                echo __( 'مبلغ پرداختی : ', 'gravityformsnavaco' ) . GFCommon::to_money( $payment_amount, rgar( $entry, "currency" ) ) . '<br/><br/>';
                echo __( 'کد رهگیری : ', 'gravityformsnavaco' ) . $transaction_id . '<br/><br/>';
                echo __( 'درگاه پرداخت : ناواکو', 'gravityformsnavaco' );
            } else {
                $payment_string = '';
                ...
                if ( $transaction_type == 1 ) {
                    if ( $payment_status != "Paid" ) {
                        $payment_string .= '<option value="Paid">' . __( 'موفق', 'gravityformsnavaco' ) . '</option>';
                    }
                }

                if ( $transaction_type == 2 ) {
                    if ( $payment_status != "Active" ) {
                        $payment_string .= '<option value="Active">' . __( 'موفق', 'gravityformsnavaco' ) . '</option>';
                    }
                }
                ...
                if ( $payment_status != "Failed" ) {
                    $payment_string .= '<option value="Failed">' . __( 'ناموفق', 'gravityformsnavaco' ) . '</option>';
                }

                if ( $payment_status != "Cancelled" ) {
                    $payment_string .= '<option value="Cancelled">' . __( 'منصرف شده', 'gravityformsnavaco' ) . '</option>';
                }

                if ( $payment_status != "Processing" ) {
                    $payment_string .= '<option value="Processing">' . __( 'معلق', 'gravityformsnavaco' ) . '</option>';
                }

                $payment_string .= '</select>';

                echo __( 'وضعیت پرداخت :', 'gravityformsnavaco' ) . $payment_string . '<br/><br/>';
                ?>
                <div id="edit_payment_status_details" style="display:block">
                    <table>
                        <tr>
                            <td><?php _e( 'تاریخ پرداخت :', 'gravityformsnavaco' ) ?></td>
                            <td><input type="text" id="payment_date" name="payment_date"
                                       value="<?php echo $payment_date ?>"></td>
                        </tr>
                        <tr>
                            <td><?php _e( 'مبلغ پرداخت :', 'gravityformsnavaco' ) ?></td>
                            <td><input type="text" id="payment_amount" name="payment_amount"
                                       value="<?php echo $payment_amount ?>"></td>
                        </tr>
                        <tr>
                            <td><?php _e( 'شماره تراکنش :', 'gravityformsnavaco' ) ?></td>
                ...
                <?php
                echo __( 'درگاه پرداخت : ناواکو (غیر قابل ویرایش)', 'gravityformsnavaco' );
            }
    ...
    public static function update_payment_entry( $form, $entry_id ) {
        ...
        global $current_user;
        $user_id   = 0;
        $user_name = __( "مهمان", 'gravityformsnavaco' );
        ...
        switch ( rgar( $entry, "payment_status" ) ) {
            case "Active" :
                $new_status = __( 'موفق', 'gravityformsnavaco' );
                break;
            ...
            case "Cancelled" :
                $new_status = __( 'منصرف شده', 'gravityformsnavaco' );
                break;

            case "Failed" :
                $new_status = __( 'ناموفق', 'gravityformsnavaco' );
                break;

            case "Processing" :
                $new_status = __( 'معلق', 'gravityformsnavaco' );
                break;
        }

        RGFormsModel::add_note( $entry["id"], $user_id, $user_name, sprintf( __( "اطلاعات تراکنش به صورت دستی ویرایش شد . وضعیت : %s - مبلغ : %s - کد رهگیری : %s - تاریخ : %s", "gravityformsnavaco" ), $new_status, GFCommon::to_money( $entry["payment_amount"], $entry["currency"] ), $payment_transaction, $entry["payment_date"] ) );
    ...
    public static function settings_page() {
        ...
        if ( ! extension_loaded( 'soap' ) ) {
            _e( 'ماژول soap بر روی سرور شما فعال نیست. برای استفاده از درگاه باید آن را فعال نمایید. با مدیر هاست تماس بگیرید.', 'gravityformsnavaco' );
        ...
        if ( rgpost( "uninstall" ) ) {
            ...
            self::uninstall();
            echo '<div class="updated fade" style="padding:20px;">' . __( "درگاه با موفقیت غیرفعال شد و اطلاعات مربوط به آن نیز از بین رفت برای فعالسازی مجدد میتوانید از طریق افزونه های وردپرس اقدام نمایید .", "gravityformsnavaco" ) . '</div>';
        ...
        if ( ! empty( $_POST ) ) {
            ...
            if ( isset( $_POST["gf_navaco_configured"] ) && ( $Response = self::Request( 'valid_checker', '', '', '' ) ) && $Response != false ) {
                ...
                if ( $Response === true ) {
                    echo '<div class="updated fade" style="padding:6px">' . __( "ارتباط با درگاه برقرار شد و اطلاعات وارد شده صحیح است .", "gravityformsnavaco" ) . '</div>';
                } else if ( $Response == 'sandbox' ) {
                    echo '<div class="updated fade" style="padding:6px">' . __( "در حالت تستی نیاز به ورود اطلاعات صحیح نمی باشد .", "gravityformsnavaco" ) . '</div>';
                } else {
                    echo '<div class="error fade" style="padding:6px">' . $Response . '</div>';
                }

            } else {
                echo '<div class="updated fade" style="padding:6px">' . __( "تنظیمات ذخیره شدند .", "gravityformsnavaco" ) . '</div>';
            }
        ...
        <form action="" method="post">
            ...
            <h3>
				...
				<i class="fa fa-credit-card"></i>
                    <?php _e( "تنظیمات ناواکو", "gravityformsnavaco" ) ?>
				</span>
            </h3>
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row"><label
                                for="gf_navaco_configured"><?php _e( "فعالسازی", "gravityformsnavaco" ); ?></label>
                    </th>
                    <td>
                        ...
                        <label class="inline"
                               for="gf_navaco_configured"><?php _e( "بله", "gravityformsnavaco" ); ?></label>
                    </td>
                </tr>

                <tr>
                    <th scope="row"><label
                                for="gf_navaco_merchent"><?php _e( "مرچنت", "gravityformsnavaco" ); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    <th scope="row"><label
                                for="gf_navaco_username"><?php _e( "نام کاربری", "gravityformsnavaco" ); ?></label>
                    </th>
                    <td>
                        <input style="width:350px;text-align:left;direction:ltr !important" type="text"
                ...
                <tr>
                    <th scope="row"><label
                                for="gf_navaco_password"><?php _e( "گذرواژه", "gravityformsnavaco" ); ?></label>
                    </th>
                    <td>
                        <input style="width:350px;text-align:left;direction:ltr !important" type="text"
                ...
                $gateway_title = __( "ناواکو", "gravityformsnavaco" );
                ...
                <tr>
                    <th scope="row">
                        <label for="gf_navaco_gname">
                            <?php _e( "عنوان", "gravityformsnavaco" ); ?>
                ...
                <tr>
                    <td colspan="2"><input style="font-family:tahoma !important;" type="submit"
                                           ...
                                           value="<?php _e( "ذخیره تنظیمات", "gravityformsnavaco" ) ?>"/></td>
                </tr>
            ...
            if ( self::has_access( "gravityforms_navaco_uninstall" ) ) {
                ...
                <div class="delete-alert alert_red">
                    ...
                    <h3>
                        ...
                        <?php _e( "غیر فعالسازی افزونه دروازه پرداخت ناواکو", "gravityformsnavaco" ); ?>
                    </h3>

                    <div
                            class="gf_delete_notice"><?php _e( "تذکر : بعد از غیرفعالسازی تمامی اطلاعات مربوط به ناواکو حذف خواهد شد", "gravityformsnavaco" ) ?></div>

                    <?php
                    $uninstall_button = '<input  style="font-family:tahoma !important;" type="submit" name="uninstall" value="' . __( "غیر فعال سازی درگاه ناواکو", "gravityformsnavaco" ) . '" class="button" onclick="return confirm(\'' . __( "تذکر : بعد از غیرفعالسازی تمامی اطلاعات مربوط به ناواکو حذف خواهد شد . آیا همچنان مایل به غیر فعالسازی میباشید؟", "gravityformsnavaco" ) . '\');"/>';
    ...
    public static function get_gname() {
        ...
        if ( isset( $settings["gname"] ) ) {
            ...
            $gname = __( 'ناواکو', 'gravityformsnavaco' );
        }
    ...
    private static function config_page() {
        ...
        <div class="wrap gforms_edit_form gf_browser_gecko">
            ...
            <h2 class="gf_admin_page_title"><?php _e( "پیکربندی درگاه ناواکو", "gravityformsnavaco" ) ?>

                <?php if ( ! empty( $_get_form_id ) ) { ?>
                    <span class="gf_admin_page_subtitle">
					<span
                            class="gf_admin_page_formid"><?php echo sprintf( __( "فید: %s", "gravityformsnavaco" ), $id ) ?></span>
					<span
                            class="gf_admin_page_formname"><?php echo sprintf( __( "فرم: %s", "gravityformsnavaco" ), $form_name ) ?></span>
				</span>
                <?php } ?>

            </h2>
            <a class="button add-new-h2" href="admin.php?page=gf_settings&subview=gf_navaco"
               style="margin:8px 9px;"><?php _e( "تنظیمات حساب ناواکو", "gravityformsnavaco" ) ?></a>
            ...
            if ( ! rgempty( "gf_navaco_submit" ) ) {
                ...
                ?>
                <div class="updated fade"
                     style="padding:6px"><?php echo sprintf( __( "فید به روز شد . %sبازگشت به لیست%s.", "gravityformsnavaco" ), "<a href='?page=gf_navaco'>", "</a>" ) ?></div>
                <?php
            }
                ...
                <div id="gf_form_toolbar">
                    <ul id="gf_form_toolbar_links">
                        ...
                        <li class="gf_form_switcher">
                            <label for="export_form"><?php _e( 'یک فید انتخاب کنید', 'gravityformsnavaco' ) ?></label>
                            <?php
                            $feeds = GFPersian_DB_navaco::get_feeds();
                            if ( RG_CURRENT_VIEW != 'entry' ) { ?>
                                <select name="form_switcher" id="form_switcher"
                                        onchange="GF_SwitchForm(jQuery(this).val());">
                                    <option value=""><?php _e( 'تغییر فید ناواکو', 'gravityformsnavaco' ) ?></option>
                                    <?php foreach ( $feeds as $feed ) {
                                        $selected = $feed["id"] == $id ? "selected='selected'" : ""; ?>
                                        <option
                                                value="<?php echo $feed["id"] ?>" <?php echo $selected ?> ><?php echo sprintf( __( 'فرم: %s (فید: %s)', 'gravityformsnavaco' ), $feed["form_title"], $feed["id"] ) ?></option>
                                    <?php } ?>
                                </select>
                                <?php
                            }
                    ...
                    <div class="gform_tab_content" id="tab_<?php echo ! empty( $current_tab ) ? $current_tab : '' ?>">
                        <div id="form_settings" class="gform_panel gform_panel_form_settings">
                            <h3>
								<span>
									<i class="fa fa-credit-card"></i>
                                    <?php _e( "پیکربندی درگاه ناواکو", "gravityformsnavaco" ); ?>
								</span>
                            </h3>
                            <form method="post" action="" id="gform_form_settings">
                                ...
                                <table class="form-table gforms_form_settings" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr style="<?php echo rgget( 'id' ) || rgget( 'fid' ) ? 'display:none !important' : ''; ?>">
                                        <th>
                                            <?php _e( "انتخاب فرم", "gravityformsnavaco" ); ?>
                                        </th>
                                        <td>
                                            <select id="gf_navaco_form" name="gf_navaco_form"
                                                    onchange="GF_SwitchFid(jQuery(this).val());">
                                                <option
                                                        value=""><?php _e( "یک فرم انتخاب نمایید", "gravityformsnavaco" ); ?> </option>
                                                <?php
                                ...
                                <?php if ( empty( $has_product ) || ! $has_product ) { ?>
                                    <div id="gf_navaco_invalid_product_form" class="gf_navaco_invalid_form"
                                        ...
                                        <?php _e( "فرم انتخاب شده هیچ گونه فیلد قیمت گذاری ندارد، لطفا پس از افزودن این فیلدها مجددا اقدام نمایید.", "gravityformsnavaco" ) ?>
                                    </div>
                                <?php } else { ?>
                                    <table class="form-table gforms_form_settings"
                                        ...
                                        <tr>
                                            <th>
                                                <?php _e( "فرم ثبت نام", "gravityformsnavaco" ); ?>
                                            </th>
                                            <td>
                                        ...
                                        <tr>
                                            <th>
                                                <?php _e( "توضیحات پرداخت", "gravityformsnavaco" ); ?>
                                            </th>
                                            <td>
                                                ...
                                                <span
                                                        class="description"><?php _e( "شورت کد ها : {form_id} , {form_title} , {entry_id}", "gravityformsnavaco" ); ?></span>
                                            </td>
                                        </tr>

                                        <tr>
                                            <th>
                                                <?php _e( "توضیح تکمیلی", "gravityformsnavaco" ); ?>
                                            </th>
                                        ...
                                        <tr>
                                            <th>
                                                <?php _e( "ایمیل پرداخت کننده", "gravityformsnavaco" ); ?>
                                            </th>
                                        ...
                                        <tr>
                                            <th>
                                                <?php _e( "تلفن همراه پرداخت کننده", "gravityformsnavaco" ); ?>
                                            </th>
                                        ...
                                        <tr <?php echo $display_post_fields ? "" : "style='display:none;'" ?>>
                                            <th>
                                                <?php _e( "نوشته بعد از پرداخت موفق", "gravityformsnavaco" ); ?>
                                            </th>
                                            <td>
                                                <select id="gf_navaco_update_action1"
                                                        name="gf_navaco_update_action1">
                                                    <option
                                                            value="default" <?php echo rgar( $config["meta"], "update_post_action1" ) == "default" ? "selected='selected'" : "" ?>><?php _e( "وضعیت پیشفرض فرم", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="publish" <?php echo rgar( $config["meta"], "update_post_action1" ) == "publish" ? "selected='selected'" : "" ?>><?php _e( "منتشر شده", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="draft" <?php echo rgar( $config["meta"], "update_post_action1" ) == "draft" ? "selected='selected'" : "" ?>><?php _e( "پیشنویس", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="pending" <?php echo rgar( $config["meta"], "update_post_action1" ) == "pending" ? "selected='selected'" : "" ?>><?php _e( "در انتظار بررسی", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="private" <?php echo rgar( $config["meta"], "update_post_action1" ) == "private" ? "selected='selected'" : "" ?>><?php _e( "خصوصی", "gravityformsnavaco" ) ?></option>
                                                </select>
                                            </td>
                                        </tr>
                                        ...
                                        <tr <?php echo $display_post_fields ? "" : "style='display:none;'" ?>>
                                            <th>
                                                <?php _e( "نوشته قبل از پرداخت موفق", "gravityformsnavaco" ); ?>
                                            </th>
                                            <td>
                                                <select id="gf_navaco_update_action2"
                                                        name="gf_navaco_update_action2">
                                                    <option
                                                            value="dont" <?php echo rgar( $config["meta"], "update_post_action2" ) == "dont" ? "selected='selected'" : "" ?>><?php _e( "عدم ایجاد پست", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="default" <?php echo rgar( $config["meta"], "update_post_action2" ) == "default" ? "selected='selected'" : "" ?>><?php _e( "وضعیت پیشفرض فرم", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="publish" <?php echo rgar( $config["meta"], "update_post_action2" ) == "publish" ? "selected='selected'" : "" ?>><?php _e( "منتشر شده", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="draft" <?php echo rgar( $config["meta"], "update_post_action2" ) == "draft" ? "selected='selected'" : "" ?>><?php _e( "پیشنویس", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="pending" <?php echo rgar( $config["meta"], "update_post_action2" ) == "pending" ? "selected='selected'" : "" ?>><?php _e( "در انتظار بررسی", "gravityformsnavaco" ) ?></option>
                                                    <option
                                                            value="private" <?php echo rgar( $config["meta"], "update_post_action2" ) == "private" ? "selected='selected'" : "" ?>><?php _e( "خصوصی", "gravityformsnavaco" ) ?></option>
                                                </select>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            <th>
                                                <?php echo __( "سازگاری با افزودنی ها", "gravityformsnavaco" ); ?>
                                            </th>
                                            <td>
                                                ...
                                                <span
                                                        class="description"><?php _e( 'برخی افزودنی های گرویتی فرم دارای متد add_delayed_payment_support هستند. در صورتی که میخواهید این افزودنی ها تنها در صورت تراکنش موفق عمل کنند این گزینه را تیک بزنید.', 'gravityformsnavaco' ); ?></span>
                                            </td>
                                        </tr>
                                        ...
                                        <tr id="gf_navaco_conditional_option">
                                            <th>
                                                <?php _e( "منطق شرطی", "gravityformsnavaco" ); ?>
                                            </th>
                                            <td>
                                                ...
                                                <label for="gf_navaco_conditional_enabled"><?php _e( "فعالسازی منطق شرطی", "gravityformsnavaco" ); ?></label><br/>
                                                <br>
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <div id="gf_navaco_conditional_container" <?php echo ! rgar( $config['meta'], 'navaco_conditional_enabled' ) ? "style='display:none'" : "" ?>>
                                                                ...
                                                                <span><?php _e( "این درگاه را فعال کن اگر ", "gravityformsnavaco" ) ?></span>

                                                                <select name="gf_navaco_conditional_type">
                                                                    <option value="all" <?php echo rgar( $config['meta'], 'navaco_conditional_type' ) == 'all' ? "selected='selected'" : "" ?>><?php _e( "همه", "gravityformsnavaco" ) ?></option>
                                                                    <option value="any" <?php echo rgar( $config['meta'], 'navaco_conditional_type' ) == 'any' ? "selected='selected'" : "" ?>><?php _e( "حداقل یکی", "gravityformsnavaco" ) ?></option>
                                                                </select>
                                                                <span><?php _e( "مطابق گزینه های زیر باشند:", "gravityformsnavaco" ) ?></span>
                                                                ...
                                                                foreach ( $condition_field_ids as $i => $value ):?>
                                                                    ...
                                                                    <div class="gf_navaco_conditional_div"
                                                                        ...
                                                                        <select class="gf_navaco_conditional_operator"
                                                                                ...
                                                                                title="">
                                                                            <option value="is"><?php _e( "هست", "gravityformsnavaco" ) ?></option>
                                                                            <option value="isnot"><?php _e( "نیست", "gravityformsnavaco" ) ?></option>
                                                                            <option value=">"><?php _e( "بیشتر یا بزرگتر از", "gravityformsnavaco" ) ?></option>
                                                                            <option value="<"><?php _e( "کمتر یا کوچکتر از", "gravityformsnavaco" ) ?></option>
                                                                            <option value="contains"><?php _e( "شامل میشود", "gravityformsnavaco" ) ?></option>
                                                                            <option value="starts_with"><?php _e( "شروع می شود با", "gravityformsnavaco" ) ?></option>
                                                                            <option value="ends_with"><?php _e( "تمام میشود با", "gravityformsnavaco" ) ?></option>
                                                                        </select>
                                                                ...
                                                                <div id="gf_no_conditional_message"
                                                                    ...
                                                                    <?php _e( "برای قرار دادن منطق شرطی، باید فیلدهای فرم شما هم قابلیت منطق شرطی را داشته باشند.", "gravityformsnavaco" ) ?>
                                                                </div>
                                        ...
                                        <tr>
                                            <td>
                                                <input type="submit" class="button-primary gfbutton"
                                                       ...
                                                       value="<?php _e( "ذخیره", "gravityformsnavaco" ); ?>"/>
                                            </td>
                                        </tr>
                                        </tbody>
                                    </table>
        ...
        <script type="text/javascript">
            ...
            function GetConditionalFieldValues(input, fieldId, selectedOperator, selectedValue, labelMaxCharacters, index) {
                ...
                if (is_text) {
                    ...
                    str += "<input type='text' class='condition_field_value' style='padding:3px' placeholder='<?php _e( "یک مقدار وارد نمایید", "gravityformsnavaco" ); ?>' id='" + input + "_value' name='" + name + "' value='" + selectedValue + "'>";
                }
                return str;
            }
    ...
    public static function Request( $confirmation, $form, $entry, $ajax ) {
        ...
        if ( ! $valid_checker ) {
            ...
            $Description = __( 'جهت بررسی صحیح بودن تنظیمات درگاه گرویتی فرم ناواکو', 'gravityformsnavaco' );
        ...
        $Message = ! empty( $Message ) ? $Message : __( 'خطایی رخ داده است.', 'gravityformsnavaco' );

        $confirmation = __( 'متاسفانه نمیتوانیم به درگاه متصل شویم. علت : ', 'gravityformsnavaco' ) . $Message;
        ...
        RGFormsModel::add_note( $entry_id, $user_id, $user_name, sprintf( __( 'خطا در اتصال به درگاه رخ داده است : %s', "gravityformsnavaco" ), $Message ) );
        ...
        if ( isset( $entry["payment_method"] ) && $entry["payment_method"] == 'navaco' ) {
            ...
            if ( $Status == 'completed' ) {
                ...
                if ( $transaction_type == 2 ) {
                    ...
                    RGFormsModel::add_note( $entry["id"], $user_id, $user_name, __( "تغییرات اطلاعات فیلدها فقط در همین پیام ورودی اعمال خواهد شد و بر روی وضعیت کاربر تاثیری نخواهد داشت .", "gravityformsnavaco" ) );
                } else {
                ...
                if ( $free == true ) {
                    ...
                    $Note = sprintf( __( 'وضعیت پرداخت : رایگان - بدون نیاز به درگاه پرداخت', "gravityformsnavaco" ) );
                } else {
                    $Note = sprintf( __( 'وضعیت پرداخت : موفق - مبلغ پرداختی : %s - کد تراکنش : %s', "gravityformsnavaco" ), $Total_Money, $transaction_id );
                }
                ...
                $Note = sprintf( __( 'وضعیت پرداخت : منصرف شده - مبلغ قابل پرداخت : %s - کد تراکنش : %s', "gravityformsnavaco" ), $Total_Money, $transaction_id );
            } else {
                $entry["payment_status"] = "Failed";
                $entry["payment_amount"] = 0;
                $entry["is_fulfilled"]   = 0;
                ...
                $Note = sprintf( __( 'وضعیت پرداخت : ناموفق - مبلغ قابل پرداخت : %s - کد تراکنش : %s - علت خطا : %s', "gravityformsnavaco" ), $Total_Money, $transaction_id, $Message );
            }
    ...
    private static function Fault( $err_code ) {
        ...
        switch((int)$err_code)
        {
            case	-1: $message = 'کلید نامعتبر است'; break;
            case	0: $message = 'تراکنش با موفقیت انجام شد.'; break;
            case	1: $message = 'صادرکننده ی کارت از انجام تراکنش صرف نظر کرد.'; break;
            case	2: $message = 'عملیات تاییدیه این تراکنش قبلا با موفقیت صورت پذیرفته است.'; break;
            case	3: $message = 'پذیرنده ی فروشگاهی نامعتبر است.'; break;
            case	5: $message = 'از انجام تراکنش صرف نظر شد.'; break;
            case	6: $message = 'بروز خطا'; break;
            case	7: $message = 'به دلیل شرایط خاص کارت توسط دستگاه ضبط شود.'; break;
            case	8: $message = 'باتشخیص هویت دارنده ی کارت، تراکنش موفق می باشد.'; break;
            case	9: $message = 'در حال حاضر امکان پاسخ دهی وجود ندارد'; break;
            case	12: $message = 'تراکنش نامعتبر است.'; break;
            case	13: $message = 'مبلغ تراکنش اصلاحیه نادرست است.'; break;
            case	14: $message = 'شماره کارت ارسالی نامعتبر است. (وجود ندارد)'; break;
            case	15: $message = 'صادرکننده ی کارت نامعتبراست.(وجود ندارد)'; break;
            case	16: $message = 'تراکنش مورد تایید است و اطلاعات شیار سوم کارت به روز رسانی شود.'; break;
            case	19: $message = 'تراکنش مجدداً ارسال شود.'; break;
            case	20: $message = 'خطای ناشناخته از سامانه مقصد'; break;
            case	23: $message = 'کارمزد ارسالی پذیرنده غیر قابل قبول است.'; break;
            case	25: $message = 'شماره شناسایی صادرکننده غیر معتبر'; break;
            case	30: $message = 'قالب پیام دارای اشکال است.'; break;
            case	31: $message = 'پذیرنده توسط سوئیچ پشتیبانی نمی شود.'; break;
            case	33: $message = 'تاریخ انقضای کارت سپری شده است'; break;
            case	34: $message = 'دارنده کارت مظنون به تقلب است.'; break;
            case	36: $message = 'کارت محدود شده است.کارت توسط دستگاه ضبط شود.'; break;
            case	38: $message = 'تعداد دفعات ورود رمز غلط بیش از حدمجاز است.'; break;
            case	39: $message = 'کارت حساب اعتباری ندارد.'; break;
            case	40: $message = 'عملیات درخواستی پشتیبانی نمی گردد.'; break;
            case	41: $message = 'کارت مفقودی می باشد.'; break;
            case	42: $message = 'کارت حساب عمومی ندارد.'; break;
            case	43: $message = 'کارت مسروقه می باشد.'; break;
            case	44: $message = 'کارت حساب سرمایه گذاری ندارد.'; break;
            case	48: $message = 'تراکنش پرداخت قبض قبلا انجام پذیرفته'; break;
            case	51: $message = 'موجودی کافی نیست.'; break;
            case	52: $message = 'کارت حساب جاری ندارد.'; break;
            case	53: $message = 'کارت حساب قرض الحسنه ندارد.'; break;
            case	54: $message = 'تاریخ انقضای کارت سپری شده است.'; break;
            case	55: $message = 'Pin-Error'; break;
            case	56: $message = 'کارت نا معتبر است.'; break;
            case	57: $message = 'انجام تراکنش مربوطه توسط دارنده ی کارت مجاز نمی باشد.'; break;
            case	58: $message = 'انجام تراکنش مربوطه توسط پایانه ی انجام دهنده مجاز نمی باشد.'; break;
            case	59: $message = 'کارت مظنون به تقلب است.'; break;
            case	61: $message = 'مبلغ تراکنش بیش از حد مجاز است.'; break;
            case	62: $message = 'کارت محدود شده است.'; break;
            case	63: $message = 'تمهیدات امنیتی نقض گردیده است.'; break;
            case	64: $message = 'مبلغ تراکنش اصلی نامعتبر است.(تراکنش مالی اصلی با این مبلغ نمی باشد)'; break;
            case	65: $message = 'تعداد درخواست تراکنش بیش از حد مجاز است.'; break;
            case	67: $message = 'کارت توسط دستگاه ضبط شود.'; break;
            case	75: $message = 'تعداد دفعات ورود رمزغلط بیش از حد مجاز است.'; break;
            case	77: $message = 'روز مالی تراکنش نا معتبر است.'; break;
            case	78: $message = 'کارت فعال نیست.'; break;
            case	79: $message = 'حساب متصل به کارت نامعتبر است یا دارای اشکال است.'; break;
            case	80: $message = 'خطای داخلی سوییچ رخ داده است'; break;
            case	81: $message = 'خطای پردازش سوییچ'; break;
            case	83: $message = 'ارائه دهنده خدمات پرداخت یا سامانه شاپرک اعلام Sign Off نموده است.'; break;
            case	84: $message = 'Host-Down'; break;
            case	86: $message = 'موسسه ارسال کننده، شاپرک یا مقصد تراکنش در حالت Sign off است.'; break;
            case	90: $message = 'سامانه مقصد تراکنش درحال انجام عملیات پایان روز می باشد.'; break;
            case	91: $message = 'پاسخی از سامانه مقصد دریافت نشد'; break;
            case	92: $message = 'مسیری برای ارسال تراکنش به مقصد یافت نشد. (موسسه های اعلامی معتبر نیستند)'; break;
            case	93: $message = 'پیام دوباره ارسال گردد. (درپیام های تاییدیه)'; break;
            case	94: $message = 'پیام تکراری است'; break;
            case	96: $message = 'بروز خطای سیستمی در انجام تراکنش'; break;
            case	97: $message = 'مبلغ تراکنش غیر معتبر است'; break;
            case	98: $message = 'شارژ وجود ندارد.'; break;
            case	99: $message = 'تراکنش غیر معتبر است یا کلید ها هماهنگ نیستند'; break;
            case	100: $message = 'خطای نامشخص'; break;
            case	500: $message = 'کدپذیرندگی معتبر نمی باشد'; break;
            case	501: $message = 'مبلغ بیشتر از حد مجاز است'; break;
            case	502: $message = 'نام کاربری و یا رمز ورود اشتباه است'; break;
            case	503: $message = 'آی پی دامنه کار بر نا معتبر است'; break;
            case	504: $message = 'آدرس صفحه برگشت نا معتبر است'; break;
            case	505: $message = 'ناشناخته'; break;
            case	506: $message = 'شماره سفارش تکراری است -  و یا مشکلی دیگر در درج اطلاعات'; break;
            case	507: $message = 'خطای اعتبارسنجی مقادیر'; break;
            case	508: $message = 'فرمت درخواست ارسالی نا معتبر است'; break;
            case	509: $message = 'قطع سرویس های شاپرک'; break;
            case	510: $message = 'لغو درخواست توسط خود کاربر'; break;
            case	511: $message = 'طولانی شدن زمان تراکنش و عدم انجام در زمان مقرر توسط کاربر'; break;
            case	512: $message = 'خطا اطلاعات Cvv2 کارت'; break;
            case	513: $message = 'خطای اطلاعات تاریخ انقضاء کارت'; break;
            case	514: $message = 'خطا در رایانامه درج شده'; break;
            case	515: $message = 'خطا در کاراکترهای کپچا'; break;
            case	516: $message = 'اطلاعات درخواست نامعتبر میباشد'; break;
            case	517: $message = 'خطا در شماره کارت'; break;
            case	518: $message = 'تراکنش مورد نظر وجود ندارد.'; break;
            case	519: $message = 'مشتری از پرداخت منصرف شده است'; break;
            case	520: $message = 'مشتری در زمان مقرر پرداخت را انجام نداده است'; break;
            case	521: $message = 'قبلا درخواست تائید با موفقیت ثبت شده است'; break;
            case	522: $message = 'قبلا درخواست اصلاح تراکنش با موفقیت ثبت شده است'; break;
            case	600: $message = 'لغو تراکنش'; break;
            case    403:$message = 'سفارش پیدا نشد'; break;
            default: $message ='خطا غیر منتظره رخ داده است';break;
        }
